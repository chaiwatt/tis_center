...
use App\Models\Bcertify\CBCbReviewerList;
...
class CheckCertificateCBController extends Controller
{
    ...
    public function create_team_review($id)
    {
        ...
        if(auth()->user()->can('add-'.$model)) {
          ...
          $previousUrl = app('url')->previous();
           ...
           $certiCb = CertiCb::find($id);
            ...
            return view('certify.cb.team-review.create',[
                'certiCb' => $certiCb ,
                'app_no' => $app_no ,
                'auditorcb' => $auditorcb,
    ...
    public function save_team_review(Request $request,$id)
    {
        $model = str_slug('auditorcb','-');
        if(auth()->user()->can('add-'.$model)) {
            $requestData = $request->all();
            $list = (array)$requestData['list'];

            foreach($list['status'] as $key => $item) {
                if($item != null){
                    $this->storeList($id,$item,
                                    $list['temp_users'][$item],
                                    $list['user_id'][$item],
                                    $list['temp_departments'][$item]
                                  );
                }
              }

              return redirect('certify/check_certificate-cb/create_team_review/'.$id)->with('flash_message', 'update ยกเลิกแต่งตั้งคณะผู้ตรวจประเมินเรียบร้อยแล้ว');
        }
        abort(403);
    }

    public function storeList($id,$status,$temp_users,$user_id,$temp_departments) {
        foreach($temp_users as $key => $itme) {
          if($itme != null){
              $input = [];
              $input['app_certi_cb_id'] = $id;
              $input['auditors_status_id'] = null;
            
              $input['status'] = $status;
              $input['temp_users'] =  $itme;
              $input['user_id'] =   $user_id[$key] ?? null;
              $input['temp_departments'] =  $temp_departments[$key] ?? null;
              CBCbReviewerList::create($input);
          }
        }
     }
  ...
  try {
    ...
    public function show($token,$id)
    {
        ...
        return view('certify/cb/check_certificate_cb.show', compact('certi_cb','history','attach_path','selectedCertiCb'));
        }
        abort(403);
    }
       ...
       // status = 2,3,5,6
        ...
        if(!is_null($certi_cb->email)){
            ...
            if(isset($certi_cb->token)){
                // dd('fggdfg');
                return redirect('certify/check_certificate-cb/'.$certi_cb->token.'/show/'.$id)->with('flash_message', 'เรียบร้อยแล้ว');
            }else{
    ...
    try {
      ...
      if($PayIn->state == null){
            ...
            if(!is_null($setting_payment) ){
                    ...
                    if (filter_var($setting_payment->data, FILTER_VALIDATE_IP)) {    
                        ...
                        $certi_cb_attach_more->file =   $this->storeFilePayinDemo($setting_payment,$certi_cb->app_no,$PayIn->auditors_id);
                    ...
                    // dd($certi_cb_attach_more->file);
                    $transaction = HP::TransactionPayIn1($PayIn->id,$tb->getTable(),'3','1',$api,$app_no.'-'.$PayIn->auditors_id);
                ...
                try {
                    ...
                    // จัดการข้อผิดพลาด เช่น บันทึกลง log หรือส่งแจ้งเตือน
                    // dd($e->getMessage());
                
                    // คุณอาจต้องการโยนข้อผิดพลาดใหม่หรือตอบสนองในวิธีที่เหมาะสม
                    // throw $e; // หรือ return response()->json(['error' => 'Unable to create CertiCbHistory'], 500);
                }
      ...
      if(!is_null($certi_cb->email)){
                ...
                if(count($certi_cb->EmailStaffAssign) > 0){
                   $data_app = [
                    ...
                    try {
                        ...
                        // คุณอาจต้องการโยนข้อผิดพลาดใหม่หรือตอบสนองในวิธีที่เหมาะสม
                        // throw $e; // หรือ return response()->json(['error' => 'Unable to send email'], 500);
                    }
         ...
         if(isset($certi_cb->token)){
            ...
            return redirect('certify/check_certificate-cb/Pay_In1/'.$id.'/'.$certi_cb->token)->with('flash_message', 'เรียบร้อยแล้ว');
         }else{
            return redirect('certify/check_certificate-cb')->with('flash_message', 'เรียบร้อยแล้ว');
        }
                 ...
                 if($PayIn->conditional_type == 1){ // เรียกเก็บค่าธรรมเนียม
                    ...
                    if(!is_null($setting_payment)){
                        ...
                        if (filter_var($setting_payment->data, FILTER_VALIDATE_IP)) {    
                            ...
                            $certi_cb_attach_more->file =   $this->storeFilePayinDemo($setting_payment,$certi_cb->app_no);
                        }
 ...
 public function storeFilePayinDemo($setting_payment, $app_no = 'files_cb', $auditor_id = '')
 {
   ...
   public function check_pay_in_cb(Request $request)
   {
       ...
       if($payin == '1'){ // pay in ครั้งที่ 1
            ...
            if(!is_null($pay_in)){
                ...
                // dd("$setting_payment->data?pid=$setting_payment->pid&out=json&Ref1=$app_no-$pay_in->auditors_id");
